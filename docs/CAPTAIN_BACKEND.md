...
Для импорта задать случайный ADMIN_TOKEN в серверном .env и перезапустить API.
Создать значение: `python -c "import secrets; print(secrets.token_urlsafe(32))"`.
Токен не помещать в Git или frontend. Скрипт читает ADMIN_TOKEN из окружения
или корневого .env; он должен совпадать с серверным значением.
...
синтетический набор. Живой вызов без предоставленных ключа/модели не подтверждён.
