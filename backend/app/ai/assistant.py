...
KNOWLEDGE = {
    'exchange': {'title': 'Обмен с 1С', 'help': 'Загрузка шести исходных XLSX одного поставщика в текущую сессию. Ключ вручную вводить не нужно. Для IEK можно дополнить inventory.csv с актуальным свободным остатком. После импорта откройте расчёт, проверьте данные, утвердите заказ и скачайте CSV в плане закупок. Это файловый обмен: совместимость с обработкой импорта конкретной базы 1С требует проверки.'},
